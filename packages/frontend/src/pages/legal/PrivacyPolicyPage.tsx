...
export default function PrivacyPolicyPage() {
  ...
  usePageSeo({
    title: `${t('privacy.title')} | TradeAnchor`,
    description: t('privacy.intro1').replace(/<[^>]+>/g, ''),
    path: '/privacy',
    lang: isZh ? 'zh-CN' : 'en',
  });
    ...
    <Box sx={{ minHeight: '100vh', bgcolor: '#0a0e17', color: '#f1f5f9', display: 'flex', flexDirection: 'column' }}>
      ...
      <Box sx={{ flex: 1, py: { xs: 6, md: 8 }, px: { xs: 2, lg: 0 } }}>
        <Container maxWidth="md">
          ...
          {/* Sections */}
          {[
            { key: 'intro', titleKey: 'privacy.section1Title' },
            { key: 'collect', titleKey: 'privacy.section2Title' },
            { key: 'usage', titleKey: 'privacy.section3Title' },
            { key: 'share', titleKey: 'privacy.section4Title' },
            { key: 'security', titleKey: 'privacy.section5Title' },
            { key: 'cookies', titleKey: 'privacy.section6Title' },
            { key: 'rights', titleKey: 'privacy.section7Title' },
            { key: 'retention', titleKey: 'privacy.section8Title' },
            { key: 'children', titleKey: 'privacy.section9Title' },
            { key: 'contact', titleKey: 'privacy.section10Title' },
          ].map((section) => (
            <Box key={section.key} sx={{ mb: 4 }}>
              <Typography variant="h6" sx={{ fontWeight: 700, color: '#00d4aa', mb: 1.5, fontSize: 16 }}>
                {t(section.titleKey)}
              </Typography>
              {Array.from({ length: 25 }, (_, i) => t(`privacy.${section.key}${i + 1}`, '')).filter(Boolean).map((text, i) => (
